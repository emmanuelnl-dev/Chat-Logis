const PRODUCTS = [
  ...
  // ── Peinture Extérieure & Façade ───────────────────────────────────────────
  {
    ...
    famille: 'Peinture Extérieure & Façade',
    supports: ['mur', 'béton'],
    usages: ['exterieur'],
    finitions: ['mat'],
...
// Interface à respecter — ne pas accéder à PRODUCTS directement depuis l'extérieur.
// Remplacer le corps de fetchProducts() par fetch('/api/products') quand l'API sera prête.
...
export async function fetchFamilles() {
  ...
  return [...new Set(products.map(p => p.famille))]
}
